...
// ========== ПЕРЕХВАТ КНОПКИ НАЗАД (ПРОФИЛЬ) ==========
window.addEventListener('load', () => {
  // Добавляем фиктивное состояние для перехвата системной кнопки "Назад"
  history.pushState({ page: 'profile' }, '', window.location.href);
});

window.addEventListener('popstate', (event) => {
  // 1. Проверяем, не открыто ли модальное окно создания поста
  const postModal = document.getElementById('postModal');
  if (postModal && postModal.style.display === 'flex') {
    postModal.style.display = 'none';
    // Возвращаем состояние, чтобы свайп назад снова сработал для выхода с самой страницы
    history.pushState({ page: 'profile' }, '', window.location.href);
    return;
  }
  
  // 2. Если модалок нет — переходим в мессенджер
