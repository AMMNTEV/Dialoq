...
function goToMessenger() {
  // Проверяем, пришли ли мы на эту страницу из мессенджера
  if (document.referrer && document.referrer.includes('messenger.html')) {
    // Если да, просто "смахиваем" текущую страницу из истории (удаляем её из стека)
    // и возвращаемся к уже существующему мессенджеру
    window.history.back();
  } else {
    // Если пользователь обновил страницу, открыл по прямой ссылке 
    // или пришел из другого места (нет истории мессенджера сзади), 
    // делаем замену, чтобы не плодить историю
